...
c.setdefault('order_types',{})['entry']='market'
c.setdefault('order_types',{})['exit']='market'
...
out={'contract':'FQT_OSV4_IP05_LOOKAHEAD_FULL_V2','csv_exists':p.exists(),'rows':rows,'row_count':len(rows),'valid_verdict':False,'has_bias':None}
if rows:
    vals=[]
    for r in rows:
